...
class GaussianPolicy(tf.keras.Model):
    ...
    def _squash_correction(self, raw_actions, stable=True, eps=1e-8):
        ...
        if not stable:
            ...
            #return tf.reduce_sum(tf.log(1. + 2.* tf.exp(2. * raw_actions)) -2. * tf.nn.softplus(2. * raw_actions), axis=1)
...
#         return tf.reduce_sum(tf.log(1.0 - tf.square(tf.tanh(raw_actions))), axis=1)
